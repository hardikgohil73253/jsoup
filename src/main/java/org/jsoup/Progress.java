...
public interface Progress<P> {
     ...
     * that is doing the work, so either don't take too long, or hand it off to another thread.
     * 
     * @param processed the number of bytes processed so far.
     * @param total     the total number of expected bytes, or -1 if unknown.
     * @param percent   the percentage of completion, 0.0..100.0. If the expected
     *                  total is unknown, % will remain at zero until complete.
